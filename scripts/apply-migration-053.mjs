#!/usr/bin/env node
/** Usage: node scripts/apply-migration-053.mjs */
...
const MIGRATION = join(root, "database", "migrations", "053_credential_earn_hint.sql");
...
async function main() {
  ...
  console.log("\n=== UNZE Migration 053 (Credential earn_hint) ===\n");
  await client.connect();
  try {
    await client.query(readFileSync(MIGRATION, "utf8"));
    console.log("053_credential_earn_hint.sql angewendet\n");
  } finally {
    await client.end();
  }
}
