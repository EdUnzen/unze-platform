...
const MIGRATION = join(root, "database", "migrations", "050_user_credential_visibility_rls.sql");
...
async function main() {
  ...
  console.log("\n=== UNZE Migration 050 (Credential Visibility RLS) ===\n");
  await client.connect();
  try {
    await client.query(sql);
    console.log("050_user_credential_visibility_rls.sql angewendet\n");
  } finally {
    await client.end();
  }
}
