#!/usr/bin/env node
/**
 * Marketing-Demo Seed ù erweitert seed:demo um 6 Vertical-Communities.
...
async function main() {
  ...
  if (process.env.UNZE_SKIP_BASE_SEED !== "true") {
    console.log("1. Basis-Demo (seed:demo)ù");
  ...
  if (!creator) {
    console.error("Demo-Creator nicht gefunden ù zuerst npm run seed:demo");
    process.exit(1);
  }
