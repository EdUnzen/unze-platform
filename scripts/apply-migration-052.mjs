#!/usr/bin/env node
/** Usage: node scripts/apply-migration-052.mjs */
...
const MIGRATION = join(root, "database", "migrations", "052_credential_archive_snapshots.sql");
...
async function main() {
  ...
  console.log("\n=== UNZE Migration 052 (Credential Archive + Snapshots) ===\n");
  await client.connect();
  try {
    await client.query(readFileSync(MIGRATION, "utf8"));
    console.log("052_credential_archive_snapshots.sql angewendet\n");
  } finally {
    await client.end();
  }
}
