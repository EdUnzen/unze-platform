...
const MIGRATION = join(root, "database", "migrations", "036_requirement_engine_phase1.sql");
...
async function main() {
  ...
  console.log("\n=== UNZE Migration 036 (Requirement Engine Phase 1) ===\n");
  await client.connect();
  try {
    await client.query(sql);
    console.log("036_requirement_engine_phase1.sql angewendet\n");
  } finally {
    await client.end();
  }
}
